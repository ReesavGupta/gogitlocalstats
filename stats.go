...
func fillCommits(email string, path string, commits map[int]int) map[int]int {
	...
	if err != nil {
		return commits // silently skip repo if it can't be opened
	}

	ref, err := repo.Head()
	if err != nil {
		// HEAD might not exist (e.g. repo initialized but no commits)
		return commits
	}
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "Error iterating commits: %v\n", err)
	}
